    ...
    if q.strip() == '' or cat == '':
        ...
        if cat == SEARCH_GROUP:
            ...
            if group_name != "":
                ...
                topics = Topic.objects.filter(group=current_group_object).filter(
                    Q(name__icontains='%s' % q) | Q(content__icontains='%s' % q))
            else:
...
def group_edit(request, gname):
    ...
    if request.method == "POST":
        ...
        Group.objects.filter(name=gname).update(name=grp_name, description=grp_intro, catelog=cate,
                                                member_nick=grp_role_mbr, modify_time=datetime.datetime.now())
        return redirect('showgroup', gname=grp_name)
